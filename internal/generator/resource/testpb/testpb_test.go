...
import (
	...
	"github.com/protoc-contrib/protoc-gen-go-aip/internal/generator/resource/testpb/external"
	"github.com/protoc-contrib/protoc-gen-go-aip/internal/generator/resource/testpb/multipattern"
	"github.com/protoc-contrib/protoc-gen-go-aip/internal/generator/resource/testpb/namefield"
	"github.com/protoc-contrib/protoc-gen-go-aip/internal/generator/resource/testpb/reference"
	"github.com/protoc-contrib/protoc-gen-go-aip/internal/generator/resource/testpb/simple"
)
...
// TestValidate exercises Validate() on the four interesting shapes:
// passing input, empty variable segment, segment containing '/', and
// the multi-pattern interface variant (which has Validate via the
// concrete struct, not the interface).
func TestValidate(t *testing.T) {
	if err := (simple.ThingName{ThingID: "foo"}).Validate(); err != nil {
		t.Errorf("valid: %v", err)
	}
	if err := (simple.ThingName{}).Validate(); err == nil || err.Error() != "thing: empty" {
		t.Errorf("empty: got %v", err)
	}
	if err := (simple.ThingName{ThingID: "a/b"}).Validate(); err == nil || err.Error() != "thing: contains illegal character '/'" {
		t.Errorf("slash: got %v", err)
	}
}

func TestTypeAndPattern(t *testing.T) {
	n := simple.ThingName{ThingID: "foo"}
	if got, want := n.Type(), "example.com/Thing"; got != want {
		t.Errorf("Type: got %q, want %q", got, want)
	}
	if got, want := n.Pattern(), "things/{thing}"; got != want {
		t.Errorf("Pattern: got %q, want %q", got, want)
	}
	// Multi-pattern variants each report their own pattern.
	pb := multipattern.PublisherBookName{PublisherID: "p", BookID: "b"}
	if got, want := pb.Pattern(), "publishers/{publisher}/books/{book}"; got != want {
		t.Errorf("PublisherBook Pattern: got %q, want %q", got, want)
	}
	ab := multipattern.AuthorBookName{AuthorID: "a", BookID: "b"}
	if got, want := ab.Pattern(), "authors/{author}/books/{book}"; got != want {
		t.Errorf("AuthorBook Pattern: got %q, want %q", got, want)
	}
}

func TestContainsWildcard(t *testing.T) {
	if (simple.ThingName{ThingID: "foo"}).ContainsWildcard() {
		t.Error("foo: expected false")
	}
	if !(simple.ThingName{ThingID: "-"}).ContainsWildcard() {
		t.Error("dash: expected true")
	}
	if !(simple.ProjectThingName{ProjectID: "-", ThingID: "t"}).ContainsWildcard() {
		t.Error("project dash: expected true")
	}
	if !(simple.ProjectThingName{ProjectID: "p", ThingID: "-"}).ContainsWildcard() {
		t.Error("thing dash: expected true")
	}
}

// TestParentConstructor exercises the constructor method emitted on the
// parent struct: parent.ChildTypeName(extraArgs...) builds the child by
// inheriting parent fields and accepting only the child-only segments.
func TestParentConstructor(t *testing.T) {
	parent := simple.ProjectName{ProjectID: "p"}
	got := parent.ProjectThingName("t")
	want := simple.ProjectThingName{ProjectID: "p", ThingID: "t"}
	if got != want {
		t.Fatalf("got %+v, want %+v", got, want)
	}
	if got.String() != "projects/p/things/t" {
		t.Errorf("String: got %q", got.String())
	}
}
